...
* ~~Форма настроек проекта в интерфейсе~~ — сделано: Настройки → «Контент-конвейер»
  (`src/components/settings/ContentPipelineSettings.tsx`): сценарий (язык, длина, тон, контекст,
  запреты, модель), HeyGen (аватар, голос, кадр, таймаут), лимиты попыток / параллельности, бюджеты,
  чат согласования; читает и пишет `content_pipeline_settings` напрямую по RLS проекта, проверка
  значений зеркалит CHECK таблицы.
