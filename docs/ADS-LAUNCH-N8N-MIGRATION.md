...
**Этап 3 — не начат.** Telegram-вход (приём из бота, сборка карусели по
`media_group_id`, анализ креатива) и выключение воркфлоу. Требует решения
владельца: перевод боевого бота на наш webhook рвёт текущий поток в момент
переключения, а после этого нужна ротация сервисного ключа и секрета
колбэка, которые лежат в теле нод.

### 3.9. Как включить нативный контур

```sql
-- по одному кабинету не переключить: флаг общий, поэтому раскатываем
-- на тестовом проекте и смотрим ad_launch_jobs
update public.automation_settings set ads_launch_native = true where id = true;
...
-- откат — одна строка, старый путь через n8n оживает сразу
