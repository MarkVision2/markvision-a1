...
Автоматически они туда **не попадают**, и это не связано с этим изменением.
В воркфлоу `supabase-deploy`:

- `supabase db push` пропускается — секрет `SUPABASE_DB_PASSWORD` не задан;
- запасной путь через Management API отвечает `HTTP 403 (error code 1010)`
  на каждый файл — у `SUPABASE_ACCESS_TOKEN` нет права `database_write`.

Поэтому edge-функции выкатываются, а таблицы под них не появляются. Пока это
так, весь набор собран одним идемпотентным файлом для SQL Editor:

```
scripts/apply-ads-native-launch.sql
```

Вставить в SQL Editor проекта и выполнить — повторный прогон безопасен.
До этого момента `launch-campaign` не находит `ads_launch_native`, считает
его выключенным и работает по старому пути через n8n: сайт не ломается, но
и прямого запуска в Meta не будет.

Чинится по-настоящему одним из двух:
- задать секрет `SUPABASE_DB_PASSWORD` — тогда работает `db push` и никакие
  списки файлов не нужны;
- выдать токену право `database_write` — тогда работает запасной путь.
