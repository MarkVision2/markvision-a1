...
**Удаление источника уносит его данные** (`radar_delete_source`, миграции
`20260909120000` / `20260909130000`). Раньше связь была `ON DELETE SET NULL`, и посты
удалённого конкурента оставались в «Трендах» без источника. Теперь RPC удаляет посты
источника, осиротевшие идеи (кроме тех, что уже стали темами контент-плана — у них
только вычищаются ссылки) и превью в bucket `radar-thumbs`; сам внешний ключ стал
`CASCADE`. Перед подтверждением интерфейс спрашивает `POST /radar/sources/:id/impact`
и показывает, сколько постов и идей исчезнет. Накопившиеся «сироты» вычищены разово:
посты без источника, чей автор не совпадает ни с одним источником проекта и которые
не приходили через «Разобрать ссылку».

**Просмотр ролика на странице** (`PostVideo`). Лента отдаёт `video_url`, поэтому на
карточке и в «рентгене» есть кнопка Play: плеер стартует по жесту пользователя и
играет со звуком (`muted` не выставляем), файл грузится только после нажатия. Ссылки
CDN площадок подписаны и через несколько дней протухают — при отказе показываем
объяснение и переход к оригиналу вместо чёрного прямоугольника.

**Плашка активных фильтров.** При сортировке «Горячее» верх ленты и так состоит из
залетевших, поэтому нажатие «Залетевшие» выглядело как «кнопка не работает». Теперь
под фильтрами появляется строка «Показаны N из M» со снятием фильтров по одному и
кнопкой «Сбросить всё».

**Сводка над вкладками (`MetricsRow`, миграция `20260909110000_radar_metrics_costs.sql`).**
Под каждым числом — расшифровка, из чего оно собрано: «всего собрано N», «из N с
X-фактором», «разобрано N», «включено из N», «сбор $0.021 · разбор $0.051». Расход берётся
из `usage_ledger` за текущий месяц и разложен на сбор (`engine in (apify, scrapecreators)`,
`note like 'radar%'`) и разбор (Whisper + LLM); раньше витрина считала только `radar_runs`,
поэтому центы округлялись в «$0.00». Суммы ниже доллара печатаются с тремя знаками
(`formatUsd`), ниже $0.001 — как «<$0.001». Кнопка «Как считаем» раскрывает формулы
X-фактора, порога залетевшего поста, оценки и источник расхода. Плитки: источники,
посты за 7 дней (+ сегодня), залетевшие, рекорд X-фактора с автором, очередь разбора,
...
* Третий заход (аудит двумя ревизорами после вкладки «Видео»): `job_retry` не сбрасывает
  контейнер у задания в `retry`, которое площадка ещё обрабатывает (иначе двойной пост), а у
  зависшего `processing` контейнер сохраняет для дожима; частичная правка группы через публичный
  API / MCP не обнуляет площадку; `tiktok-connect` не режет усечённый ответ источника по
  абсолютным смещениям; радар гасит запуск Apify, если строка `radar_runs` не записалась;
  пауза группы в `publish-intake` — 409 `group_paused`, не 500; кривое percent-кодирование в
  пути публичного API — 400 с CORS; часовой пояс аккаунта и группы проверяется по IANA и на
  сервере, и в диалоге. Фронт: фильтры очереди сбрасываются при смене проекта, гонки ответов
  закрыты номером запроса (`usePublishing`, `useRadar`), при смене фильтра старые строки не
  висят под новым чипом, ошибки загрузки заданий видны; повтор из библиотеки умеет очистить
  текст карточки и объясняет «созданo 0», когда ролик уже везде в очереди; форма «Настройки»
  не сбрасывается при перечитывании; лимит 0 отклоняется (сервер хранит от 1); поле лимита
  в меню строки не отдаёт стрелки навигации Radix; «Залить видео» ждёт загрузки аккаунтов.
* Композер: «Разные подписи для разных аккаунтов» — варианты по одному на строку уходят в
  `publish_videos.caption_variants`, `plan_publish_slots` раздаёт их по кругу (одинаковый текст в
  десятки аккаунтов площадки метят как спам); при повторе из библиотеки непустой список
  обновляет карточку. Меню «Подключить аккаунт»: у Threads / TikTok / YouTube подменю «Без
  группы / В группу …» (группы своей площадки или без площадки) — `publish-oauth` кладёт
  новый аккаунт в группу сразу после возврата с площадки.

## Теневой бан — детекция по просмотрам

Площадка о теневом бане не сообщает; наблюдаемый признак — просмотры новых постов аккаунта падают в
разы при прежнем графике. `publish_shadowban_scan(p_recent = 3, p_min_baseline = 5, p_drop = 0.3,
p_min_views = 50)` (миграция `20260909140000_publish_shadowban.sql`) по каждому аккаунту со
статусом `active` / `limited` берёт точки d1 за 60 дней: медиана просмотров по постам старше
последних трёх — «обычно», среднее по последним трём — «сейчас»; `suspect`, когда «сейчас» ниже
30 % от медианы при медиане ≥ 50. `publish-monitor mode=shadow` (крон `publish-monitor-shadow-daily`,
06:40 UTC, после ночного сбора метрик): подозрение у активного → `status = limited` с пометкой
«возможный теневой бан» в `last_error` (планировщик и claim такие аккаунты не берут, здоровье
получает штраф триггером), просмотры вернулись у аккаунта с этой пометкой → `active` обратно.
«Ограничен» руками оператора монитор не трогает. Проект получает одно сообщение в Telegram со
списком аккаунтов и цифрами. Проверено на локальном Postgres (`/tmp/pgtest/shadow.sql`): падение
1000 → 100 даёт `suspect`, ровная статистика — нет, четыре поста — данных мало.
