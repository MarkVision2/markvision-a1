...
| Кампании | пачки роликов с планом выпуска (`publish_campaigns`) |
| Видео | библиотека роликов (`publish_videos` + `publish_video_stats`): источник, задания (опубликовано / в очереди / с ошибкой), последний пост, ближайший слот; «Опубликовать ещё» — композер без заливки (`video_id`, `repost`), «Задания» — очередь с фильтром по видео, корзина — удаление ролика |
| Задания | чипы статусов со счётчиками от сервера, поиск по аккаунту / видео / ошибке, чип фильтра по видео, «Показать ещё» (до 500), колонка «Что происходит», трасса шагов, повтор и отмена; «без метрик» у постов, которых площадка больше не отдаёт |
