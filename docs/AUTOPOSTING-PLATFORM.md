...
Витрины: `publish_metrics` (проект, включая `paused`; `tokens_expiring_7d` не считает TikTok и
YouTube — их короткий access-токен продлевает `publish-monitor`, иначе плитка «Здоровье сети»
горела жёлтым постоянно, миграция `20260908090000_publish_metrics_short_tokens.sql`),
`publish_group_metrics` (по группе:
...
сети с числом аккаунтов, требующих внимания, расход против месячного бюджета. Каждая плитка —
кнопка: открывает свою вкладку и сразу нужный фильтр очереди (ошибки за сутки → «Задания» со
статусом «Ошибка»), а пояснение «что это за число» живёт в подсказке. Дальше шесть вкладок,
у каждой — строка о том, что это за сущность («Группа — расписание для пачки аккаунтов»,
«Персона — голос и лицо аккаунта»): без неё оператор видел колонку с выпадающим списком и не
понимал, что там:

| Вкладка | Что показывает |
|---|---|
| Аккаунты | один список, два вида. **Управление**: состояние, сегодня/лимит, здоровье, последний пост, включение, меню редких настроек (проверка, лимит, разгон, окно, группа, персона, отключение). Группа и персона — подписью под именем аккаунта, назначаются из меню строки. **Статистика** (витрина `publish_account_metrics`): подписчики, посты (всего / 30 дн.), показы, комментарии, ER, здоровье, сортировка по колонкам и итоги. Общие для обоих видов: поиск, чипы площадок со счётчиками, фильтр по группе, чекбоксы + панель массовых действий, «Проверить все». Таблица прокручивается внутри себя с прилипшей шапкой |
| Группы | сводка по группам (`publish_group_metrics`) над настройками групп; порог доверия `auto_publish_after` редактируется в форме, карточка пишет «одобрено подряд N из M до публикации без согласования» |
| Персоны | голос, ниша, тон, движок; нужны конвейеру тем и Reels, на публикацию готового файла не влияют |
| Видео | библиотека роликов проекта, повтор в другие аккаунты, переход к заданиям ролика |
| Задания | очередь с колонкой «Что происходит» и счётчиками статусов от сервера |
| Настройки | пауза проекта, уведомления (с кнопкой «Проверить связь»), бюджет |

**Колонка «Состояние»** говорит и статус площадки, и возьмёт ли аккаунт планировщик: «Активен»
у выключенного аккаунта или у аккаунта со здоровьем ниже 20 сопровождается строкой
«не публикует: …» с той же причиной, что вернёт `plan_publish_slots`. Причина называет статус
раньше выключателя: монитор гасит аккаунт сразу обоими (`status = error` + `publish_enabled = false`),
и «публикация выключена» отправляла чинить не то. **«Вернуть в строй»** в меню строки и в панели
массовых действий ставит `status = active` + `publish_enabled = true` (сервер заодно обнуляет серию
отказов) — до этого погашенный монитором аккаунт нельзя было оживить из интерфейса вообще.

**Колонка «Сегодня»** считает публикации за сегодняшний день в часовом поясе аккаунта. В базе
счётчик живёт парой `published_today` + `published_day` и обнуляется не в полночь, а при первой
публикации нового дня, поэтому вчерашнее «3 / 3» нельзя показывать как сегодняшнее — фронт
сбрасывает его так же, как `claim_publish_jobs` (`publishedToday` в `lib/publishingClient.ts`).

**Вкладка «Задания»** объясняет каждую строку: колонка «Что происходит» показывает разобранную
причину отказа (`jobErrorHint`: токен, лимит площадки, файл, таймаут обработки), сырой ответ
площадки и следующий шаг — в подсказке. Задание без отказа пишет, чего оно ждёт. Кнопки
«Повторить» / «Отменить» закреплены у правого края и не уезжают за границу прокрутки. Счётчики
на чипах статусов приходят из `jobs_list.counts` — по всей очереди, а не по загруженной странице
(до 500 заданий). Пачку упавших возвращает в очередь одна кнопка **«Повторить все упавшие»**
(`jobs_retry_failed`, уважает фильтр по видео) — тридцать семь кликов по строкам никто не сделает.

**Очередь встала** — отдельный сигнал: `publish_metrics.jobs_overdue` считает задания, чей слот
прошёл 15+ минут назад. Живая очередь даёт ноль (крон ходит раз в минуту), ненулевое значение
поднимает красный баннер на странице и красит плитку «В очереди»: раньше мёртвый разбор выглядел
как здоровая очередь из тринадцати заданий.

**Вкладка «Видео»** умеет убирать ролики: `video_delete` сносит карточку вместе с заданиями,
слотами, журналом и снятыми метриками (всё по `ON DELETE CASCADE`). Ролик с опубликованными постами
сервер отбивает `409 needs_force` — интерфейс спрашивает второй раз и называет, что пропадёт.
Пробные заливки копились в библиотеке навсегда.
...
| `POST /publish-accounts` `connect_threads`, `persona_*`, `settings_get/upsert`, `jobs_list` (→ `{ jobs, counts }`), `metrics`, `publish_video` | JWT / ключ | см. шапку функции |
