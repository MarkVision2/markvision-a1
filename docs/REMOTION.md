...
- **ReelsAvatar** — 1080×1920, 30 fps. Формат контент-завода: говорящая голова (аватар
  HeyGen) поверх сгенерированной анимации, четыре раскладки — `avatar_full`, `avatar_bottom`,
  `avatar_circle`, `content_full`. Видео аватара смонтировано **одним элементом** на всю
  композицию и меняет только геометрию (полный кадр → нижняя треть → круг → прозрачно):
  звук не рвётся на стыках сцен, а переходы получаются плавными. Круг лежит поверх контента,
  в остальных раскладках — под ним. Цвета берутся только из `theme` аккаунта в пропсах —
  захардкоженных цветов в компонентах нет намеренно (иначе вся пачка выходит одного цвета).
  Чистая геометрия, тайминги, титры и проверки критика — `src/avatarLayout.ts`
  (тесты `src/test/avatarLayout.test.ts` в корневом проекте, 17 штук).
  Пропсы на ролик генерирует скилл `content-factory` (этап 7), образец —
  `props/reels-avatar.example.json`. Файл аватара кладётся в `public/` под своим именем:
  `source.mp4` занят прокси монтажной съёмки.
