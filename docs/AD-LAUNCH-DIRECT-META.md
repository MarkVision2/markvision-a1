...
Тесты (`npm run test`): `adLaunchSpec` (23), `metaTargeting` (22), `adLaunchScheduler` (15), `metaGraph` (10),
`adLaunchBridge` (8) плюс расширенный `campaignWorkspace`. Весь набор — 546 тестов, зелёный;
`typecheck`, `lint` и `build` тоже проходят.
...
5. **Первый запуск сделать на тестовом кабинете.** Всё создаётся в `PAUSED`: после ответа
   ...
   до включения.
6. **Авто-запуск включается отдельно** — флажком `auto_launch_enabled` на кабинете. Пока он
   выключен, планировщик просто ничего не находит.
...
- **Расписания `ad_launch_schedules`** созданы и обрабатываются планировщиком, но UI для них нет —
  строки заводятся руками. Базовый режим (флажок на кабинете) работает без UI.
