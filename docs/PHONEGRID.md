...
### Из MarkVision: «Сетка аккаунтов → Устройства»

Edge-функция `account-devices` (клиент `src/lib/accountDevices.ts`) держит весь контур в платформе:
список, создание, прокси, питание, экран с тапами и вводом, установка приложений, привязка к аккаунту,
прогрев. Секреты — `PHONEGRID_OPEN_API_ID` и `PHONEGRID_OPEN_API_KEY` в Supabase (`docs/ENVIRONMENT.md`).

- **Изоляция проектов.** Ключ один на платформу, поэтому принадлежность телефона ведёт таблица
  `publish_devices`: проект видит свои телефоны и ничьи; ничей закрепляется за проектом при создании
  или первом действии управления (включение, привязка, экран, установка). Чужие — не видны, действие
  над ними отвечает 403.
- **Версия под шаблон прогрева** зашита в `_lib/phonegrid.ts` (`WARMUP_TEMPLATES`) и переопределяется
  парой секретов `PHONEGRID_<ПЛОЩАДКА>_WARMUP_VERSION` + `…_WARMUP_APP_VERSION_ID` без деплоя — так
  подключается TikTok, когда версия выяснена в клиенте.
- **Тапы** пересчитываются по родному разрешению кадра (`naturalWidth/Height` снимка), а не по константе.
- **Ввод текста** — только ASCII: `input text` кириллицу не принимает, интерфейс и функция это говорят прямо.
  Аргументы shell-команд идут в одинарных кавычках (`shellQuote`), чтобы `$` и кавычки не раскрывались.
- Списки телефонов читаются постранично (больше 100 устройств — не проблема); история RPA берётся
  страницей на 100 задач и фильтруется по телефону.
