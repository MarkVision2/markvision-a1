...
## Раздел «Устройства» в MarkVision

Всё, что нужно для заведения и прогрева аккаунта, живёт на вкладке «Устройства» страницы
«Сетка аккаунтов»; кабинет PhoneGrid открывать не нужно. Фронт — `components/publishing/DevicesTab.tsx`
(+ `PhoneScreenDialog`, `CreateDeviceDialog`), клиент — `lib/accountDevices.ts`, сервер —
edge-функция `account-devices` поверх чистого модуля `_lib/phonegrid.ts`. Связь телефона с
аккаунтом хранится в `publish_accounts` (`device_provider`, `device_phone_id`, `warmup_*`),
уникальный индекс не даёт посадить два аккаунта на один телефон.

Что стоит знать про поведение:

- **Список телефонов читается постранично** (`/cloudphone/page`, до 10 страниц по 100). Одна
  страница молча обрезала сеть на сотом устройстве. Так же читаются прокси и группы в форме
  создания и история RPA-прогонов.
- **История прогрева** фильтруется по телефону из общего списка `/cloudphone/rpa/subTask/page`:
  фильтра по устройству у API нет, а десяти последних записей на кабинет хватало ровно до
  второго телефона.
- **«Прогреть» доступна не всегда**: нужен привязанный аккаунт, готовый шаблон под площадку
  (`WARMUP_TEMPLATES[platform].requiredVersion`; у TikTok версия ещё не выяснена) и **выключенный**
  телефон — RPA включает его сам, иначе PhoneGrid отвечает `33309`. Кнопка гасится заранее и
  объясняет причину в подсказке: узнавать об отказе после клика поздно, телефон уже тарифицируется.
- **Перестановка телефона** на другой аккаунт сначала снимает прежний (`detach` → `attach`) и
  спрашивает подтверждение: прямая привязка упиралась в уникальный индекс.
- **Общий прокси** у нескольких телефонов подсвечивается: спокойная подсказка, если они
  выключены (у мобильного прокси адрес меняется со временем), и ошибка, если несколько
  включены одновременно — площадка видит их как один источник.
