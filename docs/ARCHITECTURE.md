...
| 4–5. Account Registry | **EXISTS, улучшить** | `publish_accounts` (+`publish_accounts_safe`), группы, здоровье 0–100 с причинами, лимиты, окна, разгон, персоны, bulk-панель в UI | Добавлены `capabilities`, `connection_type`, `auth_status` (миграция `20260909150000`) |
...
| 17. Campaigns | **MISSING → добавлено** | Ближайшее было — группа + персона + режим согласования | `publish_campaigns` + очередь + SQL-планировщик по правилу «N постов в день в заданные часы» (миграция `20260909160000`), вкладка «Кампании», API, MCP |
...
| 34–35. Routines | **MISSING → добавлено** | Кроны как «рутина» были зашиты в миграции | `publish_routines` (шаги относительно публикации) → `publish_tasks` → воркер `publish-tasks`; назначение аккаунту/группе/проекту; секция «Рутины» (миграция `20260909170000`) |
...
7. **Лимит API-ключей в памяти изолята** — «120/мин» не был глобален. Исправлено: второй предел считается по `api_request_logs` за последнюю минуту.
...
Миграция `supabase/migrations/20260909150000_content_factory_core.sql` (идемпотентна):
...
## 5a. Второй заход (Phase 2, миграция `20260909160000`)
...
## 5b. Третий заход (миграция `20260909170000`)
...
## 5e. Шестой заход — Phase 5 (миграция `20260910160000`)

- **Автопилот победителей**: `publish_replications`, `_lib/publishReplication.ts` (выбор победителей и групп —
  чистый модуль с тестами; сетевая часть зовёт конвейер), `publish-monitor mode=winner_replication` по крону
  раз в сутки для проектов с флагом `winner_replication_enabled`; ручной запуск `winner_replicate`,
  `POST /analytics/content/:id/replicate`, MCP `markvision_replicate_winner`; тумблер в настройках, кнопка
  «По группам» у лучших роликов в панели «Что работает». Цикл ТЗ «победитель → варианты → согласование →
  публикация → метрики» замкнут на существующих воротах конвейера и политике AI.
- **Метаданные контента**: `publish_videos.hook_type / cta_type / source_video_id` (+ `topic_key`), поля в
  `publish_publications`, инсайты по хукам, призывам и темам, приём по API/MCP.
- **Глобальный лимит API** — по `api_request_logs` за минуту, поверх счётчика изолята (слабое место 7).
...
**Phase 4 — закрыт** (раздел 5d). **Phase 5 — закрыт** (раздел 5e).

**Что осталось за рамками** (по убыванию пользы): Telegram-кнопки согласования для удержанных политикой
AI публикаций (сейчас — интерфейс и `POST /jobs/approve`); PKCE для TikTok; ротация ключа шифрования
`v2:`; `MEDIA_PREPARE` (ffprobe в worker/); Device engine (Phase 3) — когда появится провайдер устройств;
слой Organization над проектами.
