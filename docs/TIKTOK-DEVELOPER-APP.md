...
## Проверено вживую (репетиция 6 сентября 2026)

Прогон боевыми вызовами через `tiktok-connect` на аккаунте `@zapoinov_ai`
(песочница, client key `sbaw…`) — все продукты отвечают:

| Что | Результат |
|---|---|
| Login Kit | аккаунт подключён, `oauth_scope` содержит все 6 запрошенных прав |
| Display API `/user/info` | профиль, био, аватар, `follower_count` / `following_count` / `likes_count` / `video_count` |
| Display API `/video/list` | лента отдаётся (на момент проверки — 1 публичный ролик) |
| `creator_info/query` | ник, `privacy_level_options`, `max_video_post_duration_sec` = 3600 |
| `video.upload` (черновик) | `PROCESSING_UPLOAD` → `SEND_TO_USER_INBOX`, файл 3,3 МБ ушёл одним чанком |
| `video.publish` (Direct Post) | `PUBLISH_COMPLETE` с приватностью `SELF_ONLY` |

Что из этого важно знать до записи:

- **У аккаунта нет варианта «Все»** — `creator_info` возвращает только
  `FOLLOWER_OF_CREATOR`, `MUTUAL_FOLLOW_FRIENDS`, `SELF_ONLY` (аккаунт приватный,
  плюс ограничение неодобренного приложения). В кадре выбирайте **Only me**.
- **У приватного поста нет публичной ссылки.** TikTok отдаёт
  `publicaly_available_post_id` только для публичных видео, поэтому кнопки
  «Open the video» после Direct Post в песочнице не будет. Страница показывает
  вместо неё пояснение и кнопку «Open the profile in TikTok» — этого достаточно,
  чтобы ревьюер увидел завершённый цикл.
- **Приватный ролик не появляется в блоке 4.** `/video/list` отдаёт только
  публичные видео, поэтому сетка после публикации не изменится — не пытайтесь
  показать «вот он появился», это выглядит как сбой.
- **Дуэты и стич выключены в настройках аккаунта** — переключатели в блоке 5
  будут неактивны с подписью «disabled in the account settings». Это правильное
  поведение по UX-гайду площадки, его можно проговорить в ролике.
- **«Отключить» отзывает доступ приложения целиком.** Тот же TikTok-аккаунт
  подключён и во втором проекте («Виталя»): после демонстрации Disconnect
  переподключите его и там, иначе очередь публикаций получит `token_expired`.
- **Секрет `TIKTOK_CLIENT_SECRET` сохранён с лишним пробелом/переводом строки.**
  Код его обрезает во всех местах, но лучше пересохранить значение чисто —
  `GET /publish-oauth/diag` показывает `secret_had_whitespace: true`.
- Репетиция израсходовала две отправки из суточной квоты песочницы. Если
  публикация в кадре ответит отказом про лимит — подождите сутки.
...
**Перед включением записи обязательно отключите аккаунт** (блок 1 → Disconnect).
Если аккаунт уже подключён, TikTok на повторной авторизации пролистывает экран
согласия почти мгновенно — список прав в кадр не попадёт, а ревьюеру нужен
именно он. С пустым списком аккаунтов кнопка «Continue with TikTok» ведёт на
полноценный экран согласия со всеми шестью правами. Держите наготове файл
короткого вертикального ролика (mp4, 1080×1920) для блока 5.
...
6. **Блок 5 — Content Posting API, Direct post.** Режим «Direct post»
   ...
   TikTok» → статус «TikTok is receiving the file» → «Published». Публичной
   ссылки у приватного поста нет — страница объясняет это и предлагает «Open the
   profile in TikTok»; откройте профиль в приложении TikTok и покажите ролик там.
