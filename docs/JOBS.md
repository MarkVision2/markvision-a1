...
## Откуда ролики попадают в очередь

Три цеха, и все три теперь кладут готовый рендер в `publish_videos` — до 06.09.2026 это делал
только контент-завод, а монтаж и Reels писали лишь в свои витрины «Готовые», и опубликовать
их можно было только перезалив файл руками:

| Цех | Что пишет | `source` / `source_ref` |
|---|---|---|
| Контент-завод | `content-pipeline` при одобрении темы | `content_pipeline` / id ассета |
| AI-монтаж | `montage-worker` action `publish` (плюс каждый шортс) | `montage`, `montage-short` / `ref_id` рендера |
| Reels-видео | `reels-tts` action `publish` | `reels` / `reels-<jobId>` |

Общий шов — `_lib/publishLibrary.ts`. Он идемпотентен по паре `source` + `source_ref`:
повторный прогон того же рендера не заводит вторую карточку, иначе один ролик разошёлся бы
по аккаунтам дважды.

Статус при заведении — `ready`, то есть **в библиотеку, а не сразу в эфир**: раскладку по
аккаунтам запускает человек (или контент-завод по своей ветке через `plan_publish_slots`).
Публиковать всё, что отрендерилось, система не должна.

**Залипание ролика.** Если все задания по ролику удалили или отменили, пока он был в работе,
`settleVideo` раньше выходил молча и ролик навсегда оставался в `publishing`: закрыть его было
нечему, а разложить заново интерфейс не давал. Теперь он возвращается в `ready`.
