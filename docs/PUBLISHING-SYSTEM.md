...
| n8n-воркфлоу (копии) | `docs/n8n-publishing-*.json` |
...
Три воркфлоу созданы на `n8n.zapoinov.com`, копии — в `docs/n8n-publishing-*.json`.
**Созданы выключенными**: сначала заполните ключ, потом активируйте.

| Воркфлоу | ID | Вход |
|---|---|---|
| 📹 Публикация · 1 · Приём видео | `dRuUtSgbthYD2gC0` | `POST /webhook/publishing-video-ready` |
| 📹 Публикация · 2 · Постановка заданий | `nc2HZgFZj2M9dIgQ` | `POST /webhook/publishing-create-jobs` |
| 📹 Публикация · 3 · Очередь и мониторинг | `N4dPFNLtSSJhvBxB` | расписание, раз в 15 минут |

В каждом есть нода **«Настройки»** — единственное место с секретами:

* `automation_key` — вместо `PASTE_CRON_SECRET_HERE` вставьте
  `automation_settings.cron_secret` из Supabase;
* `tg_bot_token` и `tg_chat_id` — необязательно; пусты — отчёт в Telegram не уходит
  (уведомления об ошибках всё равно шлют edge-функции в чат проекта).

Воркфлоу 3 дублирует крон и служит отчётом: молчит, когда сказать нечего.
...
3. **n8n**: заполнить `automation_key` в ноде «Настройки» всех трёх воркфлоу и
   активировать нужные.
