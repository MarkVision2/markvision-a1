...
Он проверяет четыре независимые вещи, потому что каждая ломается отдельно:
функции (деплой edge-функций), схему (миграции — отдельный шаг деплоя, может
упасть в одиночку), ключ автоматизации и готовность подключать аккаунты
(секрет `PUBLISH_TOKEN_KEY` + живой Meta-токен проекта).
