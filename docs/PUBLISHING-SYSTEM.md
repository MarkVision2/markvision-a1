...
### App Review Meta для Instagram Login

Права `instagram_business_basic` + `instagram_business_content_publish` без расширенного доступа
работают только у аккаунтов с ролью в приложении. Что вписать в заявку, сценарий видео,
инструкции проверяющему, чеклист и тестовый маршрут через роли — `docs/INSTAGRAM-APP-REVIEW.md`.
Запрос OAuth (`INSTAGRAM_LOGIN_SCOPES`) должен совпадать со списком в заявке один в один.

### Instagram блокирует вход с компьютера

Проверено на живой ссылке 6 сентября 2026: клиент открыл приглашение на
компьютере (Mac, IP в Дубае через VPN), Instagram показал «Мы заблокировали
попытку входа» и до нашего `callback` дело не дошло. Ловить это в коде нечем:
`code` не выдан, редиректа нет, ошибку площадка не присылает — блокировка
целиком на её стороне.

Причина в том, что дверь «Вход в Instagram» ведёт на
`https://www.instagram.com/oauth/authorize`, и без активной сессии Instagram
показывает свою форму пароля. Вход по паролю с незнакомого устройства и из
чужой страны его антиугонная эвристика считает захватом аккаунта.

Лечится не кодом, а маршрутом: ссылку нужно открывать на телефоне, где
приложение уже залогинено — тогда пароль не спрашивают вовсе. Поэтому на
`/connect/:token` с экранов шире `md` показывается QR этой же ссылки
(`ConnectQrCard`), а под кнопками площадок — раскрывающаяся памятка
`ConnectBlockedHelp`: подтвердить «Это был я» в приложении, выключить VPN,
повторить с телефона. Менеджеру та же мысль напоминается в диалоге
«Подключение по ссылке». Обе подсказки — `src/components/publishing/ConnectHelp.tsx`.
