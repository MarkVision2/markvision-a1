...
Оба вебхука закрыты заголовком **`x-publish-key`** — значение в ноде
«Настройки», поле `webhook_key`. Без него заявка отклоняется: вебхук доступен
из интернета, а за ним стоит ключ автоматизации Supabase и живые аккаунты
площадок. В копии `docs/n8n-autoposting.json` вместо значения — placeholder.
...
Заявка, поставленная по ошибке, снимается из очереди до того, как крон её
разберёт:

```jsonc
{ "action": "cancel_jobs", "video_id": "uuid", "reason": "поставлено по ошибке" }
// либо точечно: { "action": "cancel_jobs", "job_ids": ["uuid", "uuid"] }
```

Снимаются только незавершённые задания (`pending` / `retry` / `processing`);
уже опубликованное не трогается. Видео, у которого не осталось незавершённых
заданий, возвращается в `ready`.

## Цель заявки: кому именно публиковать

`target.account_ids` различает «не задано» и «пустой список»:

| `target` | Кто получит видео |
|---|---|
| без `account_ids` | все активные аккаунты проекта (с фильтром `platforms`, если задан) |
| `"account_ids": []` | **никто** — заданий не создаётся |
| `"account_ids": ["…"]` | только перечисленные |
| `"group_id": "…"` | аккаунты группы; пустая группа — никто |

Пустой список раньше означал «все», и заведомо пустая тестовая заявка создала
живое задание на реальный аккаунт. Теперь это покрыто тестом.
