...
1. `next` уже скачал исходник → сделай прокси в `remotion/public/` (имя `source_<jobId8>` —
   первые 8 символов id) и веди Stage 1 → 1.5 → 2 → 4 БЕЗ чатовых ворот.
   **Цельное видео:** `delete.json` = `{"delete":[],"keep_full":true}` — НЕ резать половину
   ролика jump-cut’ами/сжатием пауз (`edl.py` keep_full). Клиент прислал готовый дубль.
   `accents.json` и **motion-вставки** (`inserts.json` через `montage-ai markup_inserts`,
   шаблоны из `docs/motion-library.md`) размечаешь обязательно — без них выходят «только титры».
   `job.brief` — вход для акцентов/вставок. `job.formats`: `16:9` → Main169; `shorts` → шортсы.
