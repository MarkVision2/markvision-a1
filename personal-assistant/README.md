...
Main bot вырос до 63 нод. Дальнейшие фичи через текущую механику добавления через MCP-канал упираются в лимит inline-PUT (~75KB inline-JSON ненадёжен в моих условиях, ломал scheduler-ноды один раз).

**Два пути дальше:**

1. **Рефактор на n8n AI Agent** (рекомендуется). Один LLM-узел в main bot с tools-описаниями. Каждый tool — это HTTP Request (или sub-call) к Supabase. Mark done, reschedule, status_query, новые интенты — добавляются как описания инструментов в одном промпте. Main bot сожмётся до ~20 нод. Расширять = редактировать промпт. Это ~полдня работы.

2. **Sub-workflows через Execute Workflow**. Каждый новый интент = отдельный маленький workflow (5-7 нод). Main bot держит только Execute Workflow ноды. Но user изначально хотел «не больше 1 workflow» — поэтому путь 1 лучше.

Готовый JSON для следующего шага (mark_done + status_query, 14 новых нод) лежит в `/tmp/main-bot-v3.json` (если sandbox ещё жив), можно либо импортировать вручную через UI n8n («⋯ → Import from File»), либо дождаться рефактора.
