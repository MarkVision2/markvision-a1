...
## video-use (сторонний скилл разговорного монтажа)
`.claude/skills/video-use/` — вендорнутый скилл [browser-use/video-use](https://github.com/browser-use/video-use):
разговорный монтаж любого видео (нарезка филлеров/дублей, колор, сабы, оверлеи). Отдельный
от нашего Montage-пайплайна инструмент; читать `SKILL.md` + `helpers/`. Зависимости тяжёлые
(librosa, scikit-learn, numba, matplotlib) — ставить лениво при первом монтаже сырья:
`.venv/bin/pip install -e .claude/skills/video-use`. Ключ Scribe — `ELEVENLABS_API_KEY` (уже
в корневом `.env`; секрет в git не коммитим, `.claude/skills/video-use/.env` в `.gitignore`).
