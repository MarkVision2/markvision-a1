 ...
 *
 * Работает Instagram. TikTok, YouTube и Threads объявлены заглушками с
...
const REGISTRY: Record<Platform, Publisher> = {
  ...
  // Threads API: POST /{user}/threads (media_type=VIDEO) → /{user}/threads_publish,
  // тот же двухшаговый порядок, что у Instagram, но отдельный токен площадки.
  threads: notImplemented("threads", "нужен доступ Threads API и токен аккаунта"),
};
