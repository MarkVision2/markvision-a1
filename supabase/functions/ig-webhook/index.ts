  ...
  try {
    for (const entry of body?.entry ?? []) {
      ...
      for (const change of entry.changes ?? []) {
        ...
        // TinyURL иногда отдаёт мобильным in-app-браузерам (Instagram) промежуточную
        // страницу вместо мгновенного редиректа — выглядело как "зависшая" ссылка.
        // Шлём собственный трек-редирект напрямую: длиннее на вид, зато без
        // стороннего непредсказуемого хопа. Клик всё так же логируется и
        // редиректит на target_url с UTM-метками.
        const link = `${SB_URL}/functions/v1/ig-organic-redirect?c=${encodeURIComponent(kw.short_id)}${username ? `&u=${encodeURIComponent(username)}` : ""}`;
