...
-- DDL идемпотентный (drop-if-exists перед create) — безопасно применять повторно.
-- ============================================================
...
drop policy if exists bc_select on public.broadcast_campaigns;
create policy bc_select on public.broadcast_campaigns
  for select using (project_id in (select id from public.projects));
drop policy if exists bc_insert on public.broadcast_campaigns;
create policy bc_insert on public.broadcast_campaigns
  for insert to authenticated with check (project_id in (select id from public.projects));
drop policy if exists bc_update on public.broadcast_campaigns;
create policy bc_update on public.broadcast_campaigns
  for update to authenticated using (project_id in (select id from public.projects))
  with check (project_id in (select id from public.projects));
drop policy if exists bc_delete on public.broadcast_campaigns;
create policy bc_delete on public.broadcast_campaigns
  for delete to authenticated using (project_id in (select id from public.projects));
create index if not exists broadcast_campaigns_project_idx on public.broadcast_campaigns (project_id, created_at desc);
create index if not exists broadcast_campaigns_active_idx on public.broadcast_campaigns (status, scheduled_at) where status in ('scheduled','sending');
...
create policy br_select on public.broadcast_recipients
  for select using (project_id in (select id from public.projects));
drop policy if exists br_insert on public.broadcast_recipients;
create policy br_insert on public.broadcast_recipients
  for insert to authenticated with check (project_id in (select id from public.projects));
drop policy if exists br_update on public.broadcast_recipients;
create policy br_update on public.broadcast_recipients
  for update to authenticated using (project_id in (select id from public.projects))
  with check (project_id in (select id from public.projects));
drop policy if exists br_delete on public.broadcast_recipients;
create policy br_delete on public.broadcast_recipients
  for delete to authenticated using (project_id in (select id from public.projects));
create index if not exists broadcast_recipients_queue_idx on public.broadcast_recipients (status, scheduled_at) where status = 'queued';
create index if not exists broadcast_recipients_campaign_idx on public.broadcast_recipients (campaign_id, status);
create index if not exists broadcast_recipients_msg_idx on public.broadcast_recipients (message_id) where message_id is not null;
create index if not exists broadcast_recipients_phone_idx on public.broadcast_recipients (project_id, phone, sent_at desc);
...
drop policy if exists bo_select on public.broadcast_opt_outs;
create policy bo_select on public.broadcast_opt_outs
  for select using (project_id in (select id from public.projects));
drop policy if exists bo_insert on public.broadcast_opt_outs;
create policy bo_insert on public.broadcast_opt_outs
  for insert to authenticated with check (project_id in (select id from public.projects));
drop policy if exists bo_delete on public.broadcast_opt_outs;
...
drop policy if exists bsd_select on public.broadcast_sender_daily;
...
drop policy if exists bss_select on public.broadcast_sender_state;
create policy bss_select on public.broadcast_sender_state
  for select using (project_id in (select id from public.projects));
drop policy if exists bss_update on public.broadcast_sender_state;
create policy bss_update on public.broadcast_sender_state
  for update to authenticated using (project_id in (select id from public.projects))
  with check (project_id in (select id from public.projects));
drop policy if exists bss_insert on public.broadcast_sender_state;
...
begin
  ...
  do update set sent = public.broadcast_sender_daily.sent + _n, updated_at = now()
...
drop trigger if exists trg_broadcast_recipients_updated on public.broadcast_recipients;
