-- Контент-центр: изоляция по проекту.
--
-- Баг: cf_content_center(p_from, p_to) читал public.instagram_media и cf_*
-- БЕЗ фильтра по project_id и вызывался edge-функцией под service role
-- (в обход RLS). В результате раздел «Контент-центр» показывал посты и
-- воронку ЧУЖИХ Instagram-аккаунтов (других проектов), смешанные с легаси
-- cf_*-данными.
--
-- Фикс: добавляем p_project_id и приводим функцию к тому же контракту, что и
-- соседние cf_content_analytics / cf_autopost_stats:
--   * p_project_id IS NOT NULL — мультиарендный путь: только instagram_media и
--     instagram_organic_events этого проекта. cf_*-воронка (клики/лиды/оплаты/
--     код-слова) привязывается по ig_media_id, который глобально уникален, так
--     что попадает только на посты этого же проекта.
--   * p_project_id IS NULL — легаси-путь content-factory (только cf_posts).
-- Edge-функция всегда передаёт project_id аутентифицированного пользователя.
...
merged_posts as (
  -- Мультиарендный путь: только посты активного проекта.
  select
  ...
  union all
  -- Легаси-путь: cf_posts без привязки к проекту — только когда проект не задан.
  select
    p.ig_media_id,
    p.caption,
    p.permalink,
...
new_post_funnel as (
  select
    ...
    count(*) filter (where e.event_type = 'link_click') as clicks,
