-- Клики по кнопке «получить доступ»: не считаем превью-краулеры Meta/мессенджеров.
-- (Контент-центр уже фильтровал; воронка код-слов — нет.)
...
COMMENT ON VIEW public.instagram_codeword_stats IS
  'Воронка по код-слову: комментарий/DM → клики (без ботов) → лиды → продажи.';
