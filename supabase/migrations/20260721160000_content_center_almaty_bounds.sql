-- Контент-центр: период в Asia/Almaty (как контент-план / cutoff 2026-07-20).
-- Раньше date::timestamptz = UTC midnight → расхождение с планом.
...
AS $function$
with b as (
  -- Границы периода в Asia/Almaty — тот же пол, что у контент-плана (не UTC midnight).
  select
    (p_from::timestamp AT TIME ZONE 'Asia/Almaty') as ts_from,
    ((p_to + 1)::timestamp AT TIME ZONE 'Asia/Almaty') as ts_to
),
merged_posts as (
  select
