-- Продолжение унификации "Контент-центра": клики/лиды/диагностика/продажи
-- по-прежнему считались только из старых cf_clicks/cf_leads/cf_payments —
-- отдельной системы, в которую новая воронка код-слов (instagram_organic_events
-- → leads с paid/amount/diagnostic_amount) ничего не пишет. Из-за этого любая
-- активность по новым код-словам (например "+") никогда не попадала в
-- Контент-центр, даже если реально были клики/лиды.
--
-- Складываем оба источника по каждому посту (legacy cf_* + новый
-- instagram_organic_events → leads), плюс отдельно считаем активность
-- код-слов "на весь аккаунт" (reel_id IS NULL — код-слово не привязано к
-- конкретному посту) в общие totals/funnel, раз её нельзя отнести к
-- конкретной строке-посту.
...
-- Новая воронка код-слов, привязанная к конкретному посту (reel_id = media_id).
new_post_funnel as (
  select
...
-- Код-слова "на весь аккаунт" (reel_id IS NULL) нельзя привязать к посту —
-- считаем отдельно и добавляем только в общие totals/funnel.
