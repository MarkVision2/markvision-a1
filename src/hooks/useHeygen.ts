...
export async function generateVideoAgent(input: VideoAgentInput): Promise<string> {
  ...
  const directives = [
    ...
    // Явное ТЗ на монтаж (тема/стиль/вставки) важнее общей директивы про
    // энергичный монтаж — если пользователь его задал, следуем именно ему.
    montageBrief
      ? `ТЗ на монтаж (приоритетно, следуй строго): ${montageBrief}`
      : "Монтаж энергичный: частая смена планов и b-roll в такт сценарию, без длинных статичных кадров с одним планом.",
  ].filter(Boolean);
