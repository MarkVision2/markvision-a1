...
export interface VideoAgentInput {
  ...
  // ТЗ на монтаж (стиль/тема/вставки — отдельно от текста для озвучки).
  // Если задано — приоритетнее общей директивы про энергичный монтаж ниже.
  montageBrief?: string;
}
...
export async function generateVideoAgent(input: VideoAgentInput): Promise<string> {
  ...
  const directives = [
    ...
    // сплошная дорожка субтитров обязательна на 100% хронометража, а плашки —
    // это ДОПОЛНЕНИЕ поверх неё, а не замена.
    "Субтитры на русском языке ОБЯЗАТЕЛЬНЫ и должны идти непрерывно на протяжении всего ролика: озвучивается фраза — на экране в этот момент обязательно должен быть подписан именно этот текст, слово в слово, без единого момента тишины субтитров.",
    ...
    "Субтитры оформляй динамично — слово/фраза появляются в такт речи, ключевые слова можно выделять цветом или жирным. Но это должна быть именно сплошная дорожка субтитров всей произносимой речи целиком. Отдельные всплывающие плашки/баннеры (например, призыв к действию) — это дополнительный элемент поверх сплошных субтитров, а не замена им: сплошные субтитры не должны пропадать на время показа такой плашки.",
    // Постоянное правило позиционирования: субтитры/текстовые плашки постоянно
    // лезли на лицо говорящего (перекрывали глаза/рот) — жёстко фиксируем зону
    // ниже центра кадра.
    "Субтитры и текстовые плашки размещай строго в нижней половине кадра — ниже вертикального центра, ближе к нижней трети. Никогда не размещай текст выше центра кадра и не закрывай им лицо говорящего (глаза, нос, рот должны быть всегда полностью видны).",
    // Плашки обрезались по краю кадра (текст не помещался и вылезал за рамку) —
    // явно требуем гарантированное умещение по ширине.
    "Любой текст в плашках, баннерах и заголовках должен целиком и полностью помещаться в кадр по ширине — ни одна буква или слово не должны обрезаться или выходить за границы экрана. Если фраза длинная — уменьшай размер шрифта или переноси её на несколько строк, но никогда не обрезай текст.",
    // Постоянное базовое требование к энергичности/музыке — раньше оно
    // подставлялось только когда ТЗ на монтаж пустое, и полностью пропадало,
    // как только пользователь задавал своё ТЗ (даже если то ТЗ ничего не
    // говорило про темп/музыку) — ролики выходили вялыми, без динамики и без
    // музыки вообще. Теперь это требование действует ВСЕГДА, а ТЗ пользователя
    // дополняет его темой/стилем, а не отменяет.
