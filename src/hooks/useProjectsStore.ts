...
function describeProjectDbError(error: PostgrestError | null, needsAuth: boolean): string {
  ...
  if (code === "PGRST205" || /Could not find the table/i.test(msg)) {
    return "Таблица projects не найдена в Supabase — примените миграции на проекте mekwfbqmsqiborjdrjxc.";
  }
  if (
    code === "42501" ||
    ...
    return "Нет прав на проект. Войдите заново или обратитесь к администратору.";
  }
  if (/intake_token/i.test(msg)) {
    return "Ошибка чтения проекта (intake_token). Обновите приложение из GitHub.";
  }
...
export function useProjectsStore() {
  ...
  const refetch = useCallback(async () => {
    const { data, error } = await supabase
      ...
      .order("created_at");
    if (error) {
      console.warn("[projects] refetch failed:", error.code, error.message);
      return;
    }
