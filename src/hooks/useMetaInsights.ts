...
function aggregate(rows: CdiRow[]): InsightsData {
  ...
  // ROMI = (выручка - расход) / расход. Берём ПОЛНУЮ выручку: CRM + ручные продажи
  // + покупки из Meta-пикселя (revenue), чтобы считать как в едином аналитическом центре.
  const fullRevenue = totals.crmRevenue + totals.revenue;
  totals.romi = totals.spend > 0
    ? ((fullRevenue - totals.spend) / totals.spend) * 100
    : fullRevenue > 0 ? 100 : 0;
