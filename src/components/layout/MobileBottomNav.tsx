import { LayoutGrid, Target, Users, GitBranch, Menu } from "lucide-react";
    ...
    >
      ...
      >
        {items.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={cn(
                "flex min-w-0 flex-col items-center justify-center gap-0.5 px-1 text-[10px] font-semibold transition-colors",
                active ? "text-success" : "text-muted-foreground",
              )}
            >
              <item.icon className={cn("h-5 w-5 shrink-0", active && "text-success")} />
        ...
        <button
          ...
          className="flex min-w-0 flex-col items-center justify-center gap-0.5 px-1 text-[10px] font-semibold text-muted-foreground transition-colors hover:text-foreground"
